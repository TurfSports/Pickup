...
class GameController {
    ...
    func loadGames(of gameType: GameType, completion: @escaping (_ games: [Game]) -> Void) {
        ...
        ref.child(gameType.name).observeSingleEvent(of: .value, with: { (snapShot) in
            DispatchQueue.main.async {
                guard let jsonObject = snapShot.value as? [String: [String: Any]] else { completion([]); print("json object not initialized on load games of game type firebase function") ; return }
    ...
    func loadGames(completion: @escaping (_ games: [Game]) -> Void) {
        ...
        ref.observeSingleEvent(of: .value, with: { (snapShot) in
            DispatchQueue.main.async {
                guard let jsonObject = snapShot.value as? [String: [String: Any]] else { completion([]); print("json object not initilized on load games firebase function") ; return }
    ...
    func loadGames(from url: URL, of gameType: GameType, completion: @escaping (_ games: [Game]) -> Void) {
        ...
        NetworkController.performRequest(for: url, httpMethod: .get, urlParameters: nil, body: nil) { (data, error) in
            DispatchQueue.main.async {
                ...
                guard let jsonObject = (try? JSONSerialization.jsonObject(with: data, options: .allowFragments)) as? [String: [String: Any]] else { completion([]); print("Json object not initialized on load games from url of gameType function") ; return }
    ...
    func loadGames(from url: URL, completion: @escaping (_ games: [Game]) -> Void) {
        ...
        NetworkController.performRequest(for: url, httpMethod: .get, urlParameters: nil, body: nil) { (data, error) in
            DispatchQueue.main.async {
                guard let data = data, error == nil else { completion([]); return }
                guard let jsonDictionary = (try? JSONSerialization.jsonObject(with: data, options: .allowFragments)) as? [String: [String: Any]] else { completion([]); print("Json object not initialized on load games from url function") ; return }
